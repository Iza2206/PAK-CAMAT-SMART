...
>
    ...
    <main class="flex-1 p-6">
        <h1 class="text-2xl font-bold mb-6 text-blue-700 dark:text-blue-300">📝 Daftar Penilaian Surat Keterangan Tanah (SKT)</h1>

        {{-- Filter --}}
        <form method="GET" action="{{ route('skt.penilaian.index') }}" class="mb-4 flex flex-wrap gap-3 items-center">
            ...
            @if(request('nik') || request('penilaian'))
                <a href="{{ route('skt.penilaian.index') }}" class="text-sm text-red-600 hover:underline">❌ Reset</a>
            @endif
        </form>

         {{-- Tombol Cetak PDF --}}
        <a href="{{ route('skt.penilaian.pdf', request()->query()) }}" target="_blank"
