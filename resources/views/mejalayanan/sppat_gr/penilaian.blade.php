...
>
    ...
    <main class="flex-1 p-6">
        <h1 class="text-2xl font-bold mb-6 text-blue-700 dark:text-blue-300">📝 Daftar Penilaian SPPAT-GR</h1>

        {{-- Filter --}}
        <form method="GET" action="{{ route('sppat_gr.penilaian.index') }}" class="mb-4 flex flex-wrap gap-3 items-center">
            ...
            @if(request('nik') || request('penilaian'))
                <a href="{{ route('sppat_gr.penilaian.index') }}" class="text-sm text-red-600 hover:underline">❌ Reset</a>
            @endif
        </form>

        {{-- Tombol Cetak PDF --}}
        <a href="{{ route('sppat_gr.penilaian.pdf', request()->query()) }}" target="_blank"
    ...
    >
        <div @click.outside="modalOpen = false" class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-sm shadow-lg">
            ...
            <form method="POST" :action="`/meja-layanan/sppat_gr/${selectedId}/penilaian`" @submit.prevent="
                if (!selectedRating) { error = true; return; }
                $el.submit();
            " class="space-y-3">
                @csrf
                <div class="grid grid-cols-2 gap-3">
                    @foreach (['tidak_puas'=>'😠 Tidak Puas', 'cukup'=>'😐 Cukup', 'puas'=>'🙂 Puas', 'sangat_puas'=>'🤩 Sangat Puas'] as $value => $label)
                        <button 
                            ...
                            class="w-full px-4 py-2 rounded text-left font-semibold transition-colors"
                        >
                            {{ $label }}
                        </button>
                    @endforeach
