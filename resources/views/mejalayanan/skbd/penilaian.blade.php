...
>
    ...
    <main class="flex-1 p-6">
        <h1 class="text-2xl font-bold mb-6 text-blue-700 dark:text-blue-300">📝 Surat Keterangan Bersih Diri (SKBD) </h1>

        {{-- Filter --}}
        <form method="GET" action="{{ route('SKBDs.penilaian.index') }}" class="mb-4 flex flex-wrap gap-3 items-center">
            ...
            @if(request('nik') || request('penilaian'))
                <a href="{{ route('SKBDs.penilaian.index') }}" class="text-sm text-red-600 hover:underline">❌ Reset</a>
            @endif
        </form>
        {{-- Tombol Cetak PDF --}}
        <a href="{{ route('SKBDs.penilaian.pdf', request()->query()) }}" target="_blank"
