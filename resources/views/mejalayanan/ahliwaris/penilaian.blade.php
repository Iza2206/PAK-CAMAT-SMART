...
>
    ...
    <main class="flex-1 p-6">
        <h1 class="text-2xl font-bold mb-6 text-blue-700 dark:text-blue-300">📝 Daftar Penilaian Ahli Waris
        </h1>

        {{-- Filter --}}
        <form method="GET" action="{{ route('ahliwaris.penilaian.index') }}" class="mb-4 flex flex-wrap gap-3 items-center">
            ...
            @if(request('nik') || request('penilaian'))
                <a href="{{ route('ahliwaris.penilaian.index') }}" class="text-sm text-red-600 hover:underline">❌ Reset</a>
            @endif
        </form>

        {{-- Tombol Cetak PDF --}}
        <a href="{{ route('ahliwaris.penilaian.pdf', request()->query()) }}" target="_blank"
    ...
    >
        <div @click.outside="modalOpen = false" class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-sm shadow-lg">
            ...
            <form 
                method="POST" 
                :action="`/meja-layanan/ahliwaris/${selectedId}/penilaian`" 
                @submit.prevent="
