...
<div x-data="{ open: false }" class="md:flex min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
    ...
    {{-- Sidebar --}}
    @include('mejalayanan.layouts.sidebar') {{-- Sidebar responsif harus gunakan yang versi sudah ada tombol close-nya --}}
    ...
    <main class="flex-1 p-6">
        <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6">
            ...
            {{-- Form --}}
            <form action="{{ route('bpjs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                ...
                @foreach ($documents as $name => $label)
                    <div>
                        <label class="block mb-1 font-semibold">{{ $label }} (PDF)</label>
                ...
                <div class="pt-4 flex items-center justify-between">
                    ...
                    <a href="{{ route('bpjs.list') }}"
                        class="text-sm text-gray-600 dark:text-gray-300 hover:underline hover:text-gray-900">
                        ← Kembali ke Daftar
                    </a>
                </div>
            </form>
