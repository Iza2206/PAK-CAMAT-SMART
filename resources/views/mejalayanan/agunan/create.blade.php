    ...
    <main class="flex-1 p-6">
        <div class="max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6">
            <h1 class="text-2xl font-bold mb-6 text-blue-700 dark:text-blue-400">
                🏦 Tambah Pengajuan Agunan ke Bank
            </h1>
            ...
            <form action="{{ route('agunan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf

                {{-- Data Pemohon --}}
                <div>
                    <label class="block mb-1 font-semibold">NIK Pemohon</label>
                    <input type="text" name="nik" maxlength="16" minlength="16" required
                        value="{{ old('nik') }}"
                ...
                @foreach([
                    'file_surat_tanah_asli' => 'Surat Tanah Asli',
                    'file_ktp' => 'Fotokopi KTP Pemohon',
                    'file_pengantar_desa' => 'Surat Pengantar dari Kepala Desa',
                    'file_surat_tidak_sengketa' => 'Surat Tidak Silang Sengketa dari Kepala Desa',
