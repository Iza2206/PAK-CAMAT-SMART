...
>
    ...
    <main class="flex-1 p-6">
        <h1 class="text-2xl font-bold mb-6 text-blue-700 dark:text-blue-300">📝 Daftar Penilaian SKTM Dispensasi Cerai</h1>

        {{-- Filter --}}
        <form method="GET" action="{{ route('sktm.penilaian.index') }}" class="mb-4 flex flex-wrap gap-3 items-center">
            ...
            @if(request('nik') || request('penilaian'))
                <a href="{{ route('sktm.penilaian.index') }}" class="text-sm text-red-600 hover:underline">❌ Reset</a>
            @endif
        </form>

        {{-- Tombol Cetak PDF --}}
        <a href="{{ route('sktm.penilaian.pdf', request()->query()) }}" target="_blank"
            ...
            <table class="min-w-full text-sm border-collapse">
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 border-t border-gray-200 dark:border-gray-600" x-data="{ showFull: false }">
    ...
    >
        <div @click.outside="modalOpen = false" class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-sm shadow-lg">
            ...
            <form method="POST" :action="`/meja-layanan/sktm/${selectedId}/penilaian`">
                @csrf
                {{-- Pilihan Penilaian --}}
