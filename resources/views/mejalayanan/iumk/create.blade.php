    ...
    <main class="flex-1 p-6">
        <div class="max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6">
            <h1 class="text-2xl font-bold mb-6 text-blue-700 dark:text-blue-400">
                📄 Tambah Pengajuan Persyaratan Izin Usaha Mikro
            </h1>
            ...
            <form action="{{ route('iumk.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    <label class="block mb-1 font-semibold">Alamat Usaha</label>
                    <textarea name="alamat_usaha" rows="3" required
                        class="w-full px-4 py-2 rounded-lg border bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('alamat_usaha') }}</textarea>
                </div>
                ...
                @foreach([
                    'surat_keterangan_usaha' => 'Surat Keterangan Usaha dari Desa',
                    'foto_tempat_usaha' => 'Foto/Gambar Tempat Usaha',
                    ...
                    <div>
                        <label class="block mb-1 font-semibold">
                            {{ $label }}
                            <span class="text-xs text-gray-500">(PDF/JPG/PNG untuk foto dan pasphoto)</span>
                        </label>
                        <input 
                            type="file" 
                            name="{{ $field }}" 
                            accept="{{ in_array($field, ['foto_tempat_usaha', 'pasphoto_3x4_1', 'pasphoto_3x4_2']) ? 'image/*' : 'application/pdf' }}"
                            required
