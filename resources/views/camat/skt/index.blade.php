...
<div class="flex flex-col md:flex-row min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
    ...
    <main class="flex-1 p-6">
        <h1 class="text-3xl font-bold mb-6 text-blue-700 dark:text-blue-300">📄 Daftar Pengajuan Surat Keterangan Tanah (SKT)</h1>
                ...
                @foreach ($pengajuan as $item)
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                        ...
                        <ul class="list-disc list-inside text-sm space-y-1 text-blue-600 dark:text-blue-300">
                            <li><a href="{{ asset('storage/' . $item->file_permohonan) }}" target="_blank">📄 Surat Permohonan</a></li>
                            <li><a href="{{ asset('storage/' . $item->file_alas_hak) }}" target="_blank">📄 Surat Alas Hak</a></li>
                            <li><a href="{{ asset('storage/' . $item->file_kk) }}" target="_blank">📄 Kartu Keluarga</a></li>
                        ...
                        <div class="mt-4 flex gap-3">
                            <form method="POST" action="{{ route('camat.skt.approve', $item->id) }}">
                                @csrf
                                <button type="submit"
                        ...
                        <dialog id="rejectModal-{{ $item->id }}" class="rounded-xl backdrop:bg-black/30 p-6 w-full max-w-md">
                            <form method="POST" action="{{ route('camat.skt.reject', $item->id) }}">
                                @csrf
