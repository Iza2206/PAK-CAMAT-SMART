                ...
                @foreach ($pengajuan as $item)
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                        ...
                        <ul class="list-disc list-inside text-sm space-y-1 text-blue-600 dark:text-blue-300">
                            ...
                            <li><a href="{{ asset('storage/' . $item->sktm) }}" target="_blank">📄 SKTM dari Desa</a></li>
                        ...
                        <div class="mt-4 flex gap-3">
                            <form method="POST" action="{{ route('camat.sktm.approve', $item->id) }}">
                                @csrf
                                <button type="submit"
                        ...
                        <dialog id="rejectModal-{{ $item->id }}" class="rounded-xl backdrop:bg-black/30 p-6 w-full max-w-md">
                            <form method="POST" action="{{ route('camat.sktm.reject', $item->id) }}">
                                @csrf
