...
<div class="flex min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
    ...
    <main class="flex-1 p-6">
        <h1 class="text-2xl font-bold text-blue-700 dark:text-blue-300 mb-6">
            📥 Verifikasi Pengajuan Surat Pernyataan Ahli Waris
        </h1>
                ...
                @foreach ($pengajuan as $item)
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                        ...
                        <div class="mt-4 flex gap-2">
                            <form method="POST" action="{{ route('kasi_pemerintahan.ahli_waris.approve', $item->id) }}">
                                @csrf
                                <button type="submit"
                        ...
                        <dialog id="rejectModal-{{ $item->id }}" class="rounded-xl backdrop:bg-black/30 p-6 w-full max-w-md">
                            <form method="POST" action="{{ route('kasi_pemerintahan.ahli_waris.reject', $item->id) }}">
                                @csrf
