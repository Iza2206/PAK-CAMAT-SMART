                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                        ...
                        <ul class="list-disc list-inside text-sm text-blue-600 dark:text-blue-300 space-y-1">
                            @if ($item->file_surat_keterangan_tanah)
                                <li><a href="{{ asset('storage/' . $item->file_surat_keterangan_tanah) }}" target="_blank">📄 Surat Keterangan Tanah</a></li>
                            @endif
                            @if ($item->file_ktp)
                                <li><a href="{{ asset('storage/' . $item->file_ktp) }}" target="_blank">📄 Fotokopi KTP</a></li>
                            @endif
                            @if ($item->file_pengantar_desa)
                                <li><a href="{{ asset('storage/' . $item->file_pengantar_desa) }}" target="_blank">📄 Surat Pengantar Kepala Desa</a></li>
                            @endif
                            @if ($item->file_pbb)
                        ...
                        <div class="mt-4 flex gap-2">
                            <form method="POST" action="{{ route('kasi_pemerintahan.skt.approve', $item->id) }}">
                                @csrf
                                <button type="submit"
                        ...
                        <dialog id="rejectModal-{{ $item->id }}" class="rounded-xl backdrop:bg-black/30 p-6 w-full max-w-md">
                            <form method="POST" action="{{ route('kasi_pemerintahan.skt.reject', $item->id) }}">
                                @csrf
