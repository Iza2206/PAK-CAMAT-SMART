...
@section('content')
    <div class="flex flex-col md:flex-row min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
        ...
        <main class="flex-1 p-6">
            <h1 class="text-3xl font-bold mb-6 text-blue-700 dark:text-blue-300">📄 Silang Sengketa</h1>
                    ...
                    @foreach ($pengajuan as $item)
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                            ...
                            <div class="mt-4 flex gap-3">
                                <form method="POST" action="{{ route('kasi_pemerintahan.silang_sengketa.approve', $item->id) }}">
                                    @csrf
                                    <button type="submit"
                            ...
                            <dialog id="rejectModal-{{ $item->id }}" class="rounded-xl backdrop:bg-black/30 p-6 w-full max-w-md">
                                <form method="POST" action="{{ route('kasi_pemerintahan.silang_sengketa.reject', $item->id) }}">
                                    @csrf
