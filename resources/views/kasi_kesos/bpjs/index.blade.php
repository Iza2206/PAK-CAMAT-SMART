                ...
                @foreach ($pengajuan as $item)
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                        ...
                        <div class="mt-4 flex gap-2">
                            <form method="POST" action="{{ route('kasi_kesos.bpjs.approve', $item->id) }}">
                                @csrf
                                <button type="submit"
                        ...
                        <dialog id="rejectModal-{{ $item->id }}" class="rounded-xl backdrop:bg-black/30 p-6 w-full max-w-md">
                            <form method="POST" action="{{ route('kasi_kesos.bpjs.reject', $item->id) }}">
                                @csrf
                                ...
                                <div class="flex justify-end mt-4 gap-2">
                                    ...
                                    <button type="button"
                                        ...
                                        class="px-4 py-2 border rounded text-sm dark:border-gray-500">
                                        Batal
                                    </button>
                                </div>
                            </form>
