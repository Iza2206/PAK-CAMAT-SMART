...
<div class="flex min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
    ...
    <main class="flex-1 p-6">
        <h1 class="text-2xl font-bold text-blue-700 dark:text-blue-300 mb-6">
            📁 Riwayat Pengajuan SKTM Dispensasi Cerai
        </h1>
                        ...
                        @foreach ($pengajuan as $index => $item)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 border-t border-gray-200 dark:border-gray-600">
                                ...
                                <td class="px-4 py-3 border text-blue-600 dark:text-blue-300 text-sm space-y-1">
                                    ...
                                    @endif
                                    @if ($item->sktm_desa)
                                        <div><a href="{{ asset('storage/' . $item->sktm_desa) }}" target="_blank">📄 SKTM Desa</a></div>
                                    @endif
                                    @if ($item->kk)
                                        <div><a href="{{ asset('storage/' . $item->kk) }}" target="_blank">📄 Kartu Keluarga</a></div>
                                    @endif
                                    @if ($item->ktp)
                                        <div><a href="{{ asset('storage/' . $item->ktp) }}" target="_blank">📄 KTP</a></div>
                                    @endif
                                    @if ($item->buku_nikah)
                                        <div><a href="{{ asset('storage/' . $item->buku_nikah) }}" target="_blank">📄 Buku Nikah</a></div>
                                    @endif
                                    @if ($item->tanda_lunas_pbb)
                                        <div><a href="{{ asset('storage/' . $item->tanda_lunas_pbb) }}" target="_blank">📄 Tanda Lunas PBB</a></div>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
