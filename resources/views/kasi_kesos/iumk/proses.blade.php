                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 border-t border-gray-200 dark:border-gray-600">
                                ...
                                <td class="px-4 py-3 border text-blue-600 dark:text-blue-300 text-sm space-y-1">
                                 @if ($item->surat_keterangan_usaha)
                                        <div>📄 <a href="{{ asset('storage/' . $item->surat_keterangan_usaha) }}" target="_blank">Surat Keterangan Usaha</a></div>
                                    @endif
                                    @if ($item->foto_tempat_usaha)
                                        <div>📄 <a href="{{ asset('storage/' . $item->foto_tempat_usaha) }}" target="_blank">Foto Tempat Usaha</a></div>
                                    @endif
                                    @if ($item->file_kk)
                                        <div>📄 <a href="{{ asset('storage/' . $item->file_kk) }}" target="_blank">Fotokopi Kartu Keluarga</a></div>
                                    @endif
                                    @if ($item->file_ktp)
                                        <div>📄 <a href="{{ asset('storage/' . $item->file_ktp) }}" target="_blank">Fotokopi KTP</a></div>
                                    @endif
