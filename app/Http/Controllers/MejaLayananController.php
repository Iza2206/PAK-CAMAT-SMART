...
public function simpanPenilaianBpjs(Request $request, $id)
{
    ...
    {
        $filters = $request->only(['nik_pemohon', 'penilaian']);
    ...
    public function simpanPenilaianiumk(Request $request, $id)
    {
        ...
        // 2. Ambil data submission
        $submission = IumkSubmission::findOrFail($id);
    ...
    // penilaian bpjs ikm
    public function penilaianiumkIndex(Request $request)
    {
        $filters = $request->only(['nik_pemohon', 'penilaian']);

        $query = \App\Models\IumkSubmission::where('status', 'approved_by_camat')
            ->filterNikStatus($filters);  // ✅ menggunakan trait
    ...
    // print out pdf
    public function penilaianIUMKPdf(Request $request)
    {
        $query = \App\Models\IumkSubmission::where('status','approved_by_camat')
            ->when($request->nik, function($q) use ($request){
                $q->where('nik_pemohon','like','%'.$request->nik.'%');
        ...
        $pdf = Pdf::loadView('mejalayanan.iumk.penilaianPdf', compact('data'))
                ->setPaper('a4', 'landscape');

        return $pdf->download('Laporan-Penilaian-IUMK.pdf');
    }
